#!/usr/bin/env python3
"""
Created on Wed Jun  7 11:20:22 2023

@author: coopy

This script will get the number of bands and kpoint folding and automatically 
add them to the inputs file in the gc_manager calculation directroy you call it
from. The same algorithm for getting number of bands and kpoint folding is
used in the suggest_input_tags.py script.
"""
...
gc_manager_path = os.environ['JDFTx_manager_home'] #get gc_manager path
...
def write_to_inputs(nbands, kpts, root):
    if type(nbands) == int:
        nbands = str(nbands)
    inputs = h.read_inputs(root)
    inputs['elec-n-bands'] = nbands
    inputs['kpoint-folding'] = kpts
...
if __name__ == '__main__':
    ...
    nbands, kpts = set_elec_n_bands(args.root, args.file, args.pseudo, args.scale, args.kptd)
    write_to_inputs(nbands, kpts, args.root)
